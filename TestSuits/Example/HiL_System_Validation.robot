...
${HOST_ALIAS}         control-baseref-100
${VIP}                192.168.0.11  # VIP IP
${OWASYS}             192.168.0.15   # OWASYS IP
${PING_COUNT}         5
${USERNAME}           root
${PASSWORD}           root
${PI_PWD}             jarvis
...
Validate Jump Server Connection and Run Ping Test
    ...
    ${return_code}    ${output}    Run And Return Rc And Output    sshpass -p ${PI_PWD} ssh -i ${PRIVATE_KEY_PATH} ${HOST_ALIAS} uname -a
...
Run Ping Test
    [Arguments]    ${ip}
    ${ping_return_code}    ${ping_output}    Run And Return Rc And Output    sshpass -p ${PI_PWD} ssh -i ${PRIVATE_KEY_PATH} ${HOST_ALIAS} ping -c ${PING_COUNT} ${ip}
...
SSH to OWASYS
    ...
    ${return_code}    ${output}    Run And Return Rc And Output    sshpass -p ${PI_PWD} ssh -i ${PRIVATE_KEY_PATH} ${HOST_ALIAS} ssh ${USERNAME}@${OWASYS} uname -a
...
SSH to VIP
    ...
    ${return_code}    ${output}    Run And Return Rc And Output    sshpass -p ${PI_PWD} ssh -i ${PRIVATE_KEY_PATH} ${HOST_ALIAS} sshpass -p ${PASSWORD} ssh ${USERNAME}@${VIP} "/proc/boot/uname -a"
